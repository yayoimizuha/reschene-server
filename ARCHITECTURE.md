...
ユーザーが JPEG 画像を S3 にアップロードすると、自動的に EXIF 情報 (GPS 座標・カメラモデル) を抽出し、メタデータを画像ごとの個別 JSON ファイルとして S3 に格納する画像管理システム。メタデータの検索・地理空間クエリには Amazon Athena を使用する。日次コンパクションジョブにより個別 JSON ファイルを Parquet に集約し、大量ファイル時のクエリ性能を確保する。さらに、同一地域の画像が一定数集まった場合に GPU インスタンス上で 3D データ再構築処理を自動実行するパイプラインを備える。
...
flowchart LR
    ...
    EXIF --> WriteJSON[個別 JSON を S3 に PUT\nraw/user_id/upload_id/file.metadata.json]
...
| 基本情報 | `user_id`            | `STRING`                                   | アップロードユーザー ID (個別 JSON ファイルに含む。全ユーザー横断クエリで使用) |
...
| ケース | 挙動 |
| --- | --- |
| EXIF 読み取り失敗 (JPEG 破損、EXIF 非搭載) | GPS・カメラ情報を `null` として個別 JSON に記録する。処理は継続し Lambda を正常終了させる |
| サムネイル生成失敗 (Pillow エラー等) | サムネイルなしとしてメタデータ書き込みは継続する。CloudWatch に警告ログを出力する |
| S3 PUT 失敗 (一時的なエラー) | Lambda を異常終了させ、S3 イベントの組み込みリトライ (最大 2 回、1 分/2 分後) に委ねる |
| 画像取得 (S3 GET) 失敗 | 同上 (異常終了 → リトライ) |

> **解決済み**: 旧 JSONL 方式の GET→PUT レースコンディションは、画像ごとの個別 JSON ファイル書き込み方式に移行したことで完全に解消された。各画像が一意の S3 キー (`raw/{user_id}/{upload_id}/{filename}.metadata.json`) に書き込むため、並列アップロード時のデータ消失は発生しない。
...
2. 対応する個別メタデータ JSON ファイル (`raw/{user_id}/{upload_id}/{filename}.metadata.json`) を `DeleteObject` で削除
3. `reschene-thumbnails` バケットの同一キーのオブジェクトを `DeleteObject` で削除 (存在しない場合は無視)

> **Note**: 画像削除時点でメタデータが既にコンパクション済み (Parquet に集約済み) の場合、レコードは Parquet 内に残り続ける (トゥームストーンフリー設計)。次回コンパクション実行時にも削除済みレコードのフィルタリングは行わない。運用上、コンパクション後の Parquet 内に削除済み画像のメタデータが含まれる場合がある。

**エラーハンドリング**:

| ケース | 挙動 |
| --- | --- |
| メタデータ JSON ファイルが存在しない | `DeleteObject` は冪等のため正常終了する |
| S3 DELETE 失敗 | Lambda を異常終了させ、S3 イベントの組み込みリトライに委ねる |
...
本システムのデータストアは **S3 (個別 JSON + Parquet コンパクション) + Athena** に統一する。

### 5.1 設計方針

- **1 画像 = 1 JSON ファイル**: メタデータ抽出 Lambda が `raw/{user_id}/{upload_id}/{filename}.metadata.json` に書き込む。各画像が一意の S3 キーを持つため、並列アップロード時のレースコンディションが発生しない
- **日次コンパクション**: EventBridge スケジュール (毎日 2:00 AM JST) で起動するコンパクション Lambda が `raw/` 配下の全 JSON ファイルを読み取り、Snappy 圧縮の Parquet ファイルとして `compacted/metadata.parquet` に集約する。集約後に元の raw ファイルを削除する
- **デュアルテーブル検索**: Athena クエリは `image_metadata_raw` テーブル (個別 JSON) と `image_metadata_compacted` テーブル (Parquet) を `UNION ALL` で横断検索する。コンパクション前後のどちらの状態でも全レコードが検索可能
- **削除**: クリーンアップ Lambda が個別 JSON ファイルを `DeleteObject` で直接削除 (読み取り不要、冪等)
...
| 操作                             | 方式                                                        |
| -------------------------------- | ----------------------------------------------------------- |
| メタデータの書き込み             | Lambda → `raw/{user_id}/{upload_id}/{file}.metadata.json` に `PutObject` |
| メタデータの削除                 | Lambda → 個別 JSON ファイルを `DeleteObject`                |
| メタデータのコンパクション       | Lambda (日次) → raw JSON 読み取り → Parquet 集約 → raw 削除 |
| ユーザーの画像一覧取得           | Athena SQL (`UNION ALL` raw + compacted, `WHERE user_id = ...`) |
| アップロードバッチの取得         | Athena SQL (`UNION ALL` raw + compacted, `WHERE upload_id = ...`) |
| 特定画像のメタデータ取得         | Athena SQL (`UNION ALL` raw + compacted, `WHERE s3_key = ...`) |
| 地理空間検索 (半径・範囲)        | Athena SQL (`UNION ALL` raw + compacted, `ST_DISTANCE`, `ST_POINT`) |

> **トレードオフ**: Athena はクエリごとに 1〜数秒のレイテンシが発生する。リアルタイム性 (ミリ秒応答) が不要な画像管理システムでは許容範囲と判断し、アーキテクチャを簡素化した。

### 5.2 メタデータの JSON 形式

**個別 JSON ファイル** (`raw/{user_id}/{upload_id}/{filename}.metadata.json`):
...
> **Note**: 1 ファイル = 1 JSON レコード。`user_id` はファイル内に含む (全ユーザー横断クエリで使用)。

### 5.3 Athena テーブル定義

**データベース**: `reschene`

検索は 2 つの Glue テーブルを `UNION ALL` で横断する:

**テーブル 1: `image_metadata_raw`** (個別 JSON ファイル)

```sql
CREATE EXTERNAL TABLE reschene.image_metadata_raw (
    user_id           STRING,
    s3_key            STRING,
    upload_id         STRING,
...
LOCATION 's3://reschene-metadata/raw/';
```

**テーブル 2: `image_metadata_compacted`** (Parquet)

```sql
CREATE EXTERNAL TABLE reschene.image_metadata_compacted (
    user_id           STRING,
    s3_key            STRING,
    upload_id         STRING,
    original_filename STRING,
...
STORED AS PARQUET
LOCATION 's3://reschene-metadata/compacted/';
```
...
```sql
SELECT * FROM (
    SELECT * FROM reschene.image_metadata_raw
    UNION ALL
    SELECT * FROM reschene.image_metadata_compacted
) AS combined
WHERE user_id = :user_id;
```

- `image_metadata_raw`: `raw/` プレフィックス配下の個別 JSON ファイルを `JsonSerDe` でパース。コンパクション前のリアルタイムデータ
- `image_metadata_compacted`: `compacted/` プレフィックス配下の Parquet ファイル。日次コンパクション済みデータ
- `user_id` は各レコードに通常カラムとして含まれる
- `WHERE user_id = '...'` を指定すると特定ユーザーの絞り込みが可能
- Athena は各テーブルの `LOCATION` 配下を再帰的にスキャン
...
| 項目                   | 対策                                                                                     |
| ---------------------- | ---------------------------------------------------------------------------------------- |
| スキャン量             | `UNION ALL` で raw (JSON) + compacted (Parquet) を横断。Parquet は列指向+Snappy 圧縮で効率的 |
| 小ファイル問題         | 日次コンパクションで個別 JSON を Parquet に集約。raw ファイルはコンパクション後に削除     |
| クエリ結果キャッシュ   | Athena のクエリ結果再利用を有効化                                                        |
| レイテンシ             | 1〜数秒は画像管理の用途では許容範囲                                                      |
| 全ユーザー横断クエリ   | 両テーブルとも全ユーザーデータを含むため、地理空間検索で横断可能                          |
| 並列安全性             | 個別 JSON ファイル方式により完全に解決。各画像が一意の S3 キーに書き込むため競合なし      |
...
| 項目               | 設定                                        |
| ------------------ | ------------------------------------------- |
| キー構造           | `raw/{user_id}/{upload_id}/{filename}.metadata.json` (個別 JSON) / `compacted/metadata.parquet` (Parquet) |
| パブリックアクセス | 全ブロック                                  |
| 暗号化             | SSE-S3 (デフォルト)                         |
...
│   │   └── handler.py
│   ├── metadata_extraction/
│   │   ├── handler.py          # EXIF 抽出 + 個別 JSON 書き込み + サムネイル生成
│   │   └── requirements.txt    # Pillow 等
│   ├── cleanup/
│   │   └── handler.py          # 個別 JSON 削除 + サムネイル削除
│   ├── search/
│   │   └── handler.py          # UNION ALL (raw + compacted) で Athena 検索
│   ├── image_url/
│   │   └── handler.py          # Presigned GET URL 発行 (追加依存なし)
│   ├── compaction/
│   │   ├── handler.py          # raw JSON → Parquet コンパクション (日次 EventBridge 起動)
...
└── tests/
    └── bulk_upload.py          # E2E テスト用並列アップロードスクリプト (ThreadPoolExecutor)
```
...
- [x] データ量増加時の Parquet コンパクション (日次 EventBridge → コンパクション Lambda で raw JSON → Parquet 集約。5.1 節参照)
...
- [x] メタデータ書き込みの並列安全性: 個別 JSON ファイル書き込み方式への移行で GET→PUT レースコンディションを完全に解消
